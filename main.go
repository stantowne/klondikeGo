...
type commandLineArgs struct {
	firstDeckNum                               int //`yaml:"firstDeckNum"`
	numberOfDecksToBePlayed                    int //`yaml:"numberOfDecksToBePlayed"`
	length                                     int //`yaml:"length"`
	verbose                                    int //`yaml:"verbose"`
	verboseSpecial                             string
	verboseSpecialProgressCounter              int
	verboseSpecialProgressCounterLastPrintTime time.Time
	findAllWinStrats                           bool            // `yaml:"findAllWinStrats"`
	pMD                                        printMoveDetail // `yaml:"pMD"`
}
	...
	/*
			...
			args[3] = length                  - of iOS (initial Override Strategy) - see comments below	(applicable to playOrig only)
		                                            ...
		                                            if length = -1 then execute playNew
		                                                      ...
		                                                      >  1 then execute playOrig and play either: the best move OR force a flip from stock to waste
								                                   >  1 is known as an iOS strategy whereby the "OR" above will be determined by  the binary representation of 2^length
