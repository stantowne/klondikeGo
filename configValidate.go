...
func configValidate(c *Configuration) {
	...
	if !(c.General.LogicVersion == "original" || c.General.LogicVersion == "multiflip1" || c.General.LogicVersion == "multiflip2" || c.General.LogicVersion == "multiflip3") {
		println("General.LogicVersion invalid; must be either 'original' or 'multiflip1' or 'multiflip2' or 'multiflip3'")
		defer os.Exit(1)
	}
	...
	if c.General.TypeOfPlay == "playOrig" {
		...
		if c.PlayAll.ProgressCounter < 0 || c.PlayAll.ProgressCounter > 100 {
			println("PlayAll.ProgressCounter invalid; must be a non-negative integer and <= 100")
			defer os.Exit(1)
		}
		if c.PlayAll.SaveResultsToSQL {
			if c.PlayAll.SQLConnectionString == "sqlDbConnect.go" {
				...
				err = db.Ping()
				if err != nil {
					fmt.Printf("Unable to verify connection to SQL using sqlDbConnect.go   err: %v\n", err)
					defer os.Exit(1)
					return
				}
				//fmt.Println("Successfully connected to SQL Server database.")
			} else {
				// example: server=DANACER\\SQLEXPRESS;port=1433;user id=@DANACER\\dan@;password=@password@;database=Klondike
				// NOTE Double BackSlashes !!!!!!!!!!!
				// Replace @...@ with actual
				var err error
				db, err = sql.Open("sqlserver", c.PlayAll.SQLConnectionString)
				if err != nil {
					fmt.Printf("Unable to connect to SQL using cfg.PlayAll.SQLConnectionString from yaml   err: %v\n", err)
					defer os.Exit(1)
					return
				}
				// Verify the connection
				err = db.Ping()
				if err != nil {
					fmt.Printf("Unable to verify connection to SQL using cfg.PlayAll.SQLConnectionString from yaml   err: %v\n", err)
					defer os.Exit(1)
					return
				}
			}
