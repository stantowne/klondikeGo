...
func validateConfig(c Configuration) {
	...
	if c.General.FirstDeckNum < 0 || c.General.FirstDeckNum > 9999 {
		println("General.FirstDeckNum invalid; must be non-negative integer less than 10,000")
		defer os.Exit(1)
	}
	if c.General.NumberOfDecksToBePlayed < 1 || c.General.NumberOfDecksToBePlayed > (10000-c.General.FirstDeckNum) {
		println("General.numberOfDecksToBePlayed invalid; must be 1 or more, but not more than 10,000 minus firstDeckNum")
		defer os.Exit(1)
	}
	...
	if c.PlayAll.ReportingType.Tree &&
		!(c.PlayAll.TreeReportingOptions.Type == "regular" ||
			c.PlayAll.TreeReportingOptions.Type == "short" ||
			c.PlayAll.TreeReportingOptions.Type == "very short") {
